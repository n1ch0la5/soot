...
export const CODEC = {
  ...
  MAGIC: 0xa55e,
  VERSION: 3,
  HEADER_BITS: 32, // 16 magic + 4 version + 12 frame count
};
...
function packHeaderBits(frames) {
...
export function writeSoundBlock(pixels, width, height, bytes, frames) {
  ...
  // header as a quiet dotted line: 1-bits are ivory dots, 0-bits stay as
  // dark card background (the reader only needs luminance at block centers)
  const bits = packHeaderBits(frames);
  ...
  for (let s = 0; s < strips; s++) {
    ...
    for (let col = 0; col < cols; col++) {
      ...
      for (let bin = 0; bin < BINS; bin++) {
        ...
        set(
          X + col,
          STRIP_Y[s] + (BINS - 1 - bin), // low frequencies at the bottom
...
export function readSoundBlock(imageData) {
  ...
  // hop/emphasis semantics changed across versions; a mismatched decode
  // would play garbled rather than fail, so reject honestly
  if (val(16, 20) !== CODEC.VERSION) throw new Error("made with a different version of soot");
  const frames = val(20, 32);
  if (!frames || frames > COLS * MAX_STRIPS) throw new Error("corrupted sound-image header");

  const bytes = new Uint8Array(frames * BINS);
  for (let f = 0; f < frames; f++) {
    const s = Math.floor(f / COLS);
    const col = f % COLS;
    const cx = (X + col + 0.5) * sx;
    for (let bin = 0; bin < BINS; bin++) {
