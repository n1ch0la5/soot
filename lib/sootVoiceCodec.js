...
const LINK = { BANDS: 22, HOP: 192, DB_RANGE: 48, MAGIC: 0x53, VERSION: 1 };
...
export async function encodeVoiceUrl(rawSamples) {
  const { FFT: N } = CODEC;
  const { BANDS, HOP, DB_RANGE } = LINK;
  const samples = conditionInput(rawSamples);
  ...
  const packed = new Uint8Array(4 + Math.ceil(delta.length / 2));
  packed[0] = LINK.MAGIC;
  packed[1] = LINK.VERSION;
  packed[2] = F & 255;
  packed[3] = F >> 8;
  for (let i = 0; i < delta.length; i++) {
    packed[4 + (i >> 1)] |= i % 2 === 0 ? delta[i] << 4 : delta[i];
  }
...
export async function decodeVoiceUrl(payload) {
  ...
  if (bytes[0] !== LINK.MAGIC || bytes[1] !== LINK.VERSION) {
    throw new Error("not a soot voice link");
  }
  const F = bytes[2] | (bytes[3] << 8);
  const { BANDS, HOP, DB_RANGE } = LINK;
  const getDelta = (i) => (i % 2 === 0 ? bytes[4 + (i >> 1)] >> 4 : bytes[4 + (i >> 1)] & 15);
  ...
  const samples = polishOutput(griffinLim(mags, 60));
